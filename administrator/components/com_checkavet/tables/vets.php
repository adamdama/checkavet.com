 ...
 * @since       11.1
 */
class JTableVets extends JTable
{
	/**
	 * Constructor
	 *
	...
	/*protected function _getAssetName()
	{
		...
		exit('get asset name');
		return 'com_checkavet.vet.'.(int) $this->$k;
	}*/
	...
	/*protected function _getAssetParentId($table = null, $id = null)
	{
		...
		// This is a vet under a category.
		if ($this->catid) {
	...
	/*public function bind($array, $ignore = '')
	{
		...
		if (isset($array['vettext'])) {
			$pattern = '#<hr\s+id=("|\')system-readmore("|\')\s*\/*>#i';
			$tagPos	= preg_match($pattern, $array['vettext']);
	...
	public function store($updateNulls = false)
	{
		...
		if ($this->id) {
			...
			// New vet. An vet created and created_by field can be set by the user,
		...
		$table = JTable::getInstance('Vets', 'JTable');
		if ($table->load(array('alias'=>$this->alias, 'id'=>$this->id)) && ($table->id != $this->id || $this->id==0)) {
			$this->setError(JText::_('JLIB_DATABASE_ERROR_VET_UNIQUE_ALIAS'));
			return false;
		}
