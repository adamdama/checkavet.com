...
?>
<form action="<?php echo JRoute::_('index.php?option=com_checkavet&view=petservices');?>" method="post" name="adminForm" id="adminForm">
	...
	<table class="adminlist">
		<thead>
			<tr>
				...
				<th width="10%">
					...
					<?php if ($saveOrder) :?>
						<?php echo JHtml::_('grid.order',  $this->items, 'filesave.png', 'petservices.saveorder'); ?>
					<?php endif; ?>
				</th>
				<th>
		...
		<?php foreach ($this->items as $i => $item) :
			...
			$canEdit	= true;//$user->authorise('core.edit',			'com_checkavet.petservice.'.$item->id);
			$canCheckin	= true;//$user->authorise('core.manage',		'com_checkin') || $item->checked_out == $userId || $item->checked_out == 0;
			$canEditOwn	= true;//$user->authorise('core.edit.own',		'com_checkavet.petservice.'.$item->id) && $item->created_by == $userId;
			$canChange	= true;//$user->authorise('core.edit.state',	'com_checkavet.petservice.'.$item->id) && $canCheckin;
			?>
			<tr class="row<?php echo $i % 2; ?>">
				<td class="center">
					<?php echo JHtml::_('grid.id', $i, $item->id); ?>
					<?php if ($item->checked_out) : ?>
						<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'petservices.', $canCheckin); ?>
					<?php endif; ?>
				</td>
				<td class="order">
					<?php if ($canChange) : ?>
						<?php if ($saveOrder) :?>
							<?php if ($listDirn == 'asc') : ?>
								<span><?php echo $this->pagination->orderUpIcon($i, true, 'petservices.orderup', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
								<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, true, 'petservices.orderdown', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
							<?php elseif ($listDirn == 'desc') : ?>
								<span><?php echo $this->pagination->orderUpIcon($i, true, 'petservices.orderdown', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
								<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, true, 'petservices.orderup', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
							<?php endif; ?>
						<?php endif; ?>
				...
				<td>
					<?php if ($canEdit || $canEditOwn) : ?>
						<a href="<?php echo JRoute::_('index.php?option=com_checkavet&task=petservice.edit&id='.$item->id);?>">
				...
				<td class="center">
					<?php echo JHtml::_('jgrid.published', $item->state, $i, 'petservices.', $canChange, 'cb', $item->publish_up, $item->publish_down); ?>
				</td>
				<td class="center">
